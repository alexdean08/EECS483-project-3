...
bool Expr::Check(bool reportError){
    ...
    if(dynamic_cast<ReadIntegerExpr*>(this) != nullptr){
      type = Type::intType;
    }
    if(dynamic_cast<ReadLineExpr*>(this) != nullptr){
...
bool ArrayAccess::Check(bool reportError){
    ...
    if(dynamic_cast<ArrayType*>(base->type) == nullptr){
        if(reportError && !base->type->IsEquivalentTo(Type::errorType))
    ...
    if(!subscript->type->IsEquivalentTo(Type::intType) && !subscript->type->IsEquivalentTo(Type::errorType)){
        if(reportError)
    ...
    if(base==NULL){
        ...
        while(true) {
            ...
            if(dynamic_cast<Program*>(parent) != nullptr) {
                ...
                if(dynamic_cast<FieldAccess*>(base) == nullptr && dynamic_cast<This*>(base) == nullptr){
...
bool Call::Check(bool reportError) {
    ...
    if(base==NULL){
        ...
        base->Check(reportError);
        //printf("%s\n", base->type->typeName);
        if(base->type != NULL && dynamic_cast<ArrayType*>(base->type) != nullptr){
            //printf("%s\n", base->type->typeName);
        ...
        while(true) {
            ...
            if(dynamic_cast<Program*>(parent) != nullptr) {
                ...
                if(dynamic_cast<ClassDecl*>(temp_cl) == nullptr && dynamic_cast<InterfaceDecl*>(temp_cl) == nullptr){
                    // ERROR did not find class or interface
                    if(reportError)
                        ReportError::FieldNotFoundInBase(field, base->type);
                    type=Type::errorType;
                    return false;
                ...
                Decl *temp_fn_decl = temp_cl->CheckHash(field);
...
void Call::validateFormals(Identifier *FnIdentifier, List<VarDecl*> *decl_formals, List<Expr*> *call_actuals, bool reportError){
    ...
    for(int i = 0; i < min_elements; ++i){
        ...
        NamedType* call_actuals_nt = dynamic_cast<NamedType*>(call_actuals->Nth(i)->type);
        NamedType* decl_formals_nt = dynamic_cast<NamedType*>(decl_formals->Nth(i)->type);

        if(call_actuals_nt != NULL && decl_formals_nt != NULL) {
            if(!call_actuals_nt->isType(decl_formals_nt)) {
                ReportError::ArgMismatch(call_actuals->Nth(i), i+1, call_actuals->Nth(i)->type, decl_formals->Nth(i)->type);
            }
        }
        else if( !( decl_formals->Nth(i)->type->IsEquivalentTo(call_actuals->Nth(i)->type) || decl_formals->Nth(i)->type->IsEquivalentTo(Type::errorType) || call_actuals->Nth(i)->type->IsEquivalentTo(Type::errorType) || call_actuals->Nth(i)->type->IsEquivalentTo(Type::nullType)) ){
            if(reportError)
                ReportError::ArgMismatch(call_actuals->Nth(i), i+1, call_actuals->Nth(i)->type, decl_formals->Nth(i)->type);
        }
    }
...
bool NewExpr::Check(bool reportError){
    ...
    bool valid = cType->CheckNew(reportError);
    ...
    if(!valid){
        ...
        Identifier *i = new Identifier(*cType->GetLocation(), strdup(cType->typeName));
        if(dynamic_cast<InterfaceDecl*>(parent->CheckHash(i)) != NULL) {
            type = Type::errorType;
...
bool NewArrayExpr::Check(bool reportError) {
    ...
    if(!valid){
        ...
        if(dynamic_cast<NamedType*>(elemType) != nullptr){
            ...
            Identifier *i = new Identifier(*temp_type->GetLocation(), strdup(temp_type->typeName));
            if(dynamic_cast<InterfaceDecl*>(parent->CheckHash(i)) != NULL) {
                type = Type::errorType;
                ReportError::IdentifierNotDeclared(i, LookingForClass);
            }
...
bool ArithmeticExpr::Check(bool reportError){
    ...
    if(left == NULL){
        right->Check(false);
        if(right->type->IsEquivalentTo(Type::errorType)) {
            type = Type::errorType;
        }
        else if(!right->type->IsEquivalentTo(Type::intType) && !right->type->IsEquivalentTo(Type::doubleType)) {
            if(reportError)
